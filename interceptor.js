 ...
 */
(function() {
  console.log('[Tracker Inject v1.4.6] Initializing API Interceptors in MAIN world...');
  ...
  window.fetch = async function(...args) {
    ...
    if (isTargetApi(url)) {
      try {
        if (!response.ok) {
          console.warn('[Tracker Inject v1.4.6] Fetch API error status:', response.status, 'URL:', url);
          window.postMessage({ type: 'PUROLATOR_API_ERROR', url, status: response.status }, '*');
        } else {
          const clone = response.clone();
          clone.json().then(data => {
            console.log('[Tracker Inject v1.4.6] Fetch captured JSON response from:', url, data);
            window.postMessage({ type: 'PUROLATOR_API_RESPONSE', url, data }, '*');
          }).catch(err => {
            console.error('[Tracker Inject v1.4.6] Fetch JSON parse err:', err, 'URL:', url);
            window.postMessage({ type: 'PUROLATOR_API_ERROR', url, status: 405, message: '数据解析失败' }, '*');
          });
        }
      } catch (e) {
        console.error('[Tracker Inject v1.4.6] Fetch intercept err:', e, 'URL:', url);
      }
    }
    return response;
  };
      ...
      this.addEventListener('load', () => {
        try {
          if (this.status >= 200 && this.status < 300) {
            ...
            console.log('[Tracker Inject v1.4.6] XHR captured JSON response from:', url, data);
            window.postMessage({ type: 'PUROLATOR_API_RESPONSE', url, data }, '*');
          } else {
            console.warn('[Tracker Inject v1.4.6] XHR API error status:', this.status, 'URL:', url);
            window.postMessage({ type: 'PUROLATOR_API_ERROR', url, status: this.status }, '*');
          }
        } catch (err) {
          console.error('[Tracker Inject v1.4.6] XHR response JSON parse err:', err, 'URL:', url);
          window.postMessage({ type: 'PUROLATOR_API_ERROR', url, status: this.status || 405, message: 'XHR数据解析失败' }, '*');
        }
      });

      this.addEventListener('error', () => {
        console.error('[Tracker Inject v1.4.6] XHR request network error. URL:', url);
  ...
  console.log('[Tracker Inject v1.4.6] Fetch and XHR Interceptors successfully initialized.');
})();
