...
console.log('[Tracker Content v1.4.8] Content Script initialized in ISOLATED world.');
...
window.addEventListener('message', (event) => {
  if (event.data?.type === 'PUROLATOR_API_RESPONSE') {
    console.log('[Tracker Content v1.4.8] Intercepted API success response, URL:', event.data.url);
    chrome.runtime.sendMessage({
      action: 'API_RESPONSE_CAPTURED',
      url: event.data.url,
      data: event.data.data
    }).catch(err => console.warn('[Tracker Content v1.4.8] Success message forward failed:', err));
  } else if (event.data?.type === 'PUROLATOR_API_ERROR') {
    console.log('[Tracker Content v1.4.8] Intercepted API error response, URL:', event.data.url);
    chrome.runtime.sendMessage({
      action: 'API_RESPONSE_ERROR',
      url: event.data.url,
      status: event.data.status,
      message: event.data.message || '官方接口请求失败'
    }).catch(err => console.warn('[Tracker Content v1.4.8] Error message forward failed:', err));
  }
});
  ...
  for (const s of selectors) {
    ...
    for (const el of elements) {
      ...
      if (el && el.offsetWidth > 0 && el.offsetHeight > 0 && !isHeaderOrGlobalSearch(el)) {
        console.log('[Tracker Content v1.4.8] Matched input element selector:', s);
        return el;
      }
    }
  }
...
function findTrackButton() {
  ...
  for (const d of direct) {
    if (d && d.offsetWidth > 0 && d.offsetHeight > 0 && !isHeaderOrGlobalSearch(d)) {
      console.log('[Tracker Content v1.4.8] Matched direct button selector.');
      return d;
    }
  }
  ...
  for (const el of candidates) {
    ...
    if (txt === 'track' || txt === 'track now' || txt === 'track package' || txt === 'search' || txt === '查询') {
      if (el.offsetWidth > 0 && el.offsetHeight > 0) {
        console.log('[Tracker Content v1.4.8] Matched button text exact:', txt);
        return el;
      }
    }
  }
  ...
  for (const el of candidates) {
    ...
    if (txt.includes('track') || txt.includes('search') || txt.includes('查询')) {
      if (el.offsetWidth > 0 && el.offsetHeight > 0) {
        console.log('[Tracker Content v1.4.8] Matched button text partial:', txt);
        return el;
      }
    }
  }
...
async function executeSearchFlow(trackingId) {
  console.log('[Tracker Content v1.4.8] Starting DOM automation for ID:', trackingId);
  ...
  try {
    ...
    console.log(`[Tracker Content v1.4.8] DOM Scan: Found ${allInputs.length} total input/textarea tags.`);
    ...
    console.warn('[Tracker Content v1.4.8] DOM diagnostic scan failed:', e.message);
  }

  // Poll for tracking input box (up to 10 seconds) to wait for React/dynamic framework rendering
  let input = null;
  const inputStart = Date.now();
  console.log('[Tracker Content v1.4.8] Polling for tracking input box in body...');
  ...
  if (!input) {
    console.error('[Tracker Content v1.4.8] Failed to find main tracking input box in body (Timeout 10s)!');
    throw new Error('未找到单号输入框，请确保页面已完全加载完毕。');
  }
  console.log('[Tracker Content v1.4.8] Target Input Box Found:', input.tagName, 'ID:', input.id, 'Placeholder:', input.placeholder);
  ...
  if (btn) {
    console.log('[Tracker Content v1.4.8] Target Button Found:', btn.tagName, 'ID:', btn.id, 'Text:', (btn.textContent || btn.value || '').trim());
  } else {
    console.warn('[Tracker Content v1.4.8] No specific search button found outside header. Will attempt form submit or Enter keypress fallbacks.');
  }
  ...
  if (btn) {
    ...
    console.log('[Tracker Content v1.4.8] Triggering click on Search target:', clickTarget.tagName, 'Class:', clickTarget.className);
    simulateMouseClick(clickTarget);
  }

  // Double Insurance: Dispatch Enter keypress on input to force form submission
  await new Promise(r => setTimeout(r, 200));
  console.log('[Tracker Content v1.4.8] Dispatching Enter keypress on input to ensure form submission...');
